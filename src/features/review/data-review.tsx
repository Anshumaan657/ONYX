  ...
  return <section aria-labelledby="review-title">
    ...
    {reviewNeeded ? <div className="mt-5 rounded-2xl border border-amber-500/30 bg-amber-500/[.07] p-5" role="status">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        <div className="rounded-xl border border-(--line) bg-(--surface) p-4"><p className="text-sm font-bold">Needs attention</p><ul className="mt-2 space-y-2 text-sm text-(--muted)">{blocking.slice(0, 5).map((issue, index) => <li key={`${issue.section}-${issue.rowId ?? index}`}>— {issueLabel(issue)}</li>)}{hasWorkbookIssues && <li>— Workbook compatibility has warnings to review</li>}</ul>{blocking.length > 5 && <p className="setup-help">And {blocking.length - 5} more items</p>}</div>
        <div className="rounded-xl border border-(--line) bg-(--surface) p-4"><p className="text-sm font-bold">Optional improvements</p><ul className="mt-2 space-y-2 text-sm text-(--muted)">{warnings.slice(0, 5).map((issue, index) => <li key={`${issue.section}-${issue.rowId ?? index}`}>— {issueLabel(issue)}</li>)}{!warnings.length && <li>— No optional improvements detected</li>}</ul></div>
      </div>
    ...
    </div> : <div className="mt-5 rounded-2xl border border-teal-500/30 bg-teal-500/[.07] p-5"><h2 className="font-bold">Nothing needs your attention</h2><p className="mt-2 text-sm text-(--muted)">The workbook is ready. Choose a date or date range to view the financial summary.</p><button className="setup-button mt-4" type="button" onClick={onResults}>Choose dates and view results</button></div>}
